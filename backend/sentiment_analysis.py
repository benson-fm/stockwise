...
def generate_response(contents: list[str], stock: str, sample_size: int, model: object) -> tuple[str, float, str, str]:
    ...
    try:
        ...
        response_action = model.generate_content(f"From the following advice, {response.text}, return me either, buy, sell, or hold, no explanation, either buy, sell, or hold, one word")
        ...
        price_per_stock = model.generate_content(f"What is the current stock price of {stock}? Return to me only the cost.")
        ...
        return advice_text, avg_polarity, action, positive, neutral, negative, price_per_stock
